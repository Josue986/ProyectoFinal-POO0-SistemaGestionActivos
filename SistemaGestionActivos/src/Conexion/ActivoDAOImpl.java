...
public class ActivoDAOImpl implements ActivoDAO {
    ...
    public boolean guardar(Activo activo) {
        ...
        try {
            ...
            if (activo instanceof Cpu) {
                ...
                // CORREGIDO: Se ajustaron los parámetros a 4 marcadores '?' reales
                String sqlMouse = "INSERT INTO perifericos (idActivo, anniosUso, tipoConexion, dpi, resolucion, tasaDeRefresco) VALUES (?, ?, ?, ?, NULL, NULL);";
                ...
                // CORREGIDO: Ajustados los índices de asignación para que coincidan secuencialmente con los marcadores '?' reales (1, 2, 3, 4, 5)
                String sqlMonitor = "INSERT INTO perifericos (idActivo, anniosUso, tipoConexion, dpi, resolucion, tasaDeRefresco) VALUES (?, ?, ?, NULL, ?, ?);";
                try (PreparedStatement psMonitor = conn.prepareStatement(sqlMonitor)) {
                    ...
                    psMonitor.setString(4, mon.getResolucion()); // El 4° '?' de la consulta corresponde a resolucion
                    psMonitor.setString(5, mon.getTasaDeRefresco()); // El 5° '?' corresponde a tasaDeRefresco
    ...
    @Override
        public boolean eliminar(int id) { // <-- Cambiado a int
        ...
        try {
            ...
            if (conn != null) {
                ...
                ps.setInt(1, id); // <-- CORREGIDO: Usar setInt en lugar de setString
